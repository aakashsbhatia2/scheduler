...
struct sched_attr {
	...
	/* New sjnprio fields */
	uint64_t esfp_start_time;
	uint64_t esfp_priority;
	uint64_t esfp_id;
};
...
void run_child(int n) {
	...
	attr.sched_policy = SCHED_ESFP;
	attr.esfp_id = child_id;
	// children have deadlines ranges from 2s down to 400ms
	// later children have sooner deadlines, so should schedule first
	attr.esfp_start_time = 2000 * millis -  child_id * 100 * millis; //later children have sooner deadlines
	
	if(attr.esfp_id == 15){
		attr.esfp_start_time = 1;
		attr.esfp_priority = 100;
	}
	else if(attr.esfp_id == 14){
                attr.esfp_start_time = 1;
                attr.esfp_priority = 150;
	}
	else {
		attr.esfp_start_time = n+15;
		attr.esfp_priority = -1;
	}
...
int main() {
	printf("Casio test program, 16 tasks\n");
